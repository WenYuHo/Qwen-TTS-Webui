...
## 2026-03-25 - [Audio Player Consolidation and State Feedback]
**Learning:** Consolidating audio playback from multiple disparate components to a single, global persistent player (like the sidebar player) simplifies the mental model for the user and ensures a consistent monitoring experience. Providing immediate visual feedback (loading spinners) on the play button while the audio is being initialized prevents the "frozen UI" feeling during network latency.
**Action:** Route all component-level audio previews through the global #preview-player. Always implement loading states and dynamic ARIA labels for media controls.
