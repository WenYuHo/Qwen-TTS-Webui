...
class AudioPostProcessor:
    ...
    def apply_declick(wav: np.ndarray, sr: int) -> np.ndarray:
        ...
        try:
            ...
            # ⚡ Bolt: Process main body in vectorized chunks to eliminate O(N) Python loops
            num_chunks = len(wav) // window
            main_len = num_chunks * window
            main_wav = wav[:main_len].reshape(num_chunks, window)

            # ⚡ Bolt: Use np.einsum for vectorized RMS calculation per chunk.
            # This avoids intermediate array allocations and is significantly faster.
            rms = np.sqrt(np.einsum('ij,ij->i', main_wav, main_wav) / window) + 1e-6

            # Identify spikes (> 10x local RMS)
            # ⚡ Bolt: Use broadcasting for O(1) threshold comparison
            threshold = rms[:, np.newaxis] * 10
            spikes = np.abs(main_wav) > threshold

            if np.any(spikes):
                out_main = main_wav.copy()
                # ⚡ Bolt: Use np.broadcast_to to expand RMS values for spike clamping
                rms_expanded = np.broadcast_to(rms[:, np.newaxis], (num_chunks, window))
                out_main[spikes] = np.sign(main_wav[spikes]) * rms_expanded[spikes] * 3
            ...
            # Return a copy via concatenation
