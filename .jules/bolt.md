...
## 2026-03-21 - [Vectorized Audio De-clicking]
**Learning:** Replacing O(N) Python loops with vectorized NumPy operations for signal processing (like de-clicking) provides massive speedups (e.g., ~20x). Using `np.reshape` and `np.einsum` for sliding-window heuristics is significantly more efficient than manual iteration. Using boolean masks for selective replacement (`out[spikes] = values`) avoids unnecessary array copying.
**Action:** Always prefer vectorized NumPy patterns for window-based audio heuristics. Use `np.einsum` for memory-efficient squared sums and RMS calculations to avoid large temporary array allocations.
