...
class AudioPostProcessor:
    ...
    def apply_declick(wav: np.ndarray, sr: int) -> np.ndarray:
        """⚡ Bolt: Vectorized de-clicker: reduces O(N) Python loops to O(1) NumPy calls."""
        try:
            if len(wav.shape) > 1:
                # ⚡ Bolt: Process each channel independently via vectorized stack
            ...
            # ⚡ Bolt: Split into full windows for block-vectorized processing
            n_full = len(wav) // window
            wav_full = wav[:n_full * window]
            remainder = wav[n_full * window:]

            chunks = wav_full.reshape(n_full, window)
            # ⚡ Bolt: Use einsum for squared sum to avoid allocating chunks**2
            rms = np.sqrt(np.einsum('ij,ij->i', chunks, chunks) / window) + 1e-6
            
            # ⚡ Bolt: Identify spikes across all blocks simultaneously
            spikes = np.abs(chunks) > (rms[:, None] * 10)

            if np.any(spikes):
                # Only copy and modify if spikes exist to save memory
                out_full = chunks.copy()
                # ⚡ Bolt: Use broadcast_to for memory-efficient indexing
                rms_broadcast = np.broadcast_to(rms[:, None], chunks.shape)
                out_full[spikes] = np.sign(chunks[spikes]) * rms_broadcast[spikes] * 3
                wav_out = out_full.ravel()
            else:
                wav_out = wav_full.copy()

            # Process remainder if it's large enough to have an RMS
            if len(remainder) >= 2:
            ...
            return np.concatenate([wav_out, remainder]) if len(remainder) > 0 else wav_out
        except Exception as e:
