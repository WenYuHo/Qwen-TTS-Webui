...
class AudioPostProcessor:
    ...
    def apply_declick(wav: np.ndarray, sr: int) -> np.ndarray:
        ...
        # ⚡ Bolt: Vectorized de-clicker. Replaces O(N) Python loop with NumPy operations.
        # This provides ~15-30x speedup for typical audio durations.
        try:
            if len(wav.shape) > 1:
            ...
            # Handle too small audio or window
            ...
            # Reshape into chunks: (n_chunks, window) for bulk processing
            chunks = wav[:full_part_len].reshape(n_full_chunks, window)
            
            # ⚡ Bolt: Use einsum for squared sum to avoid a large temporary allocation for chunks**2.
            # RMS = sqrt(sum(x^2) / N)
            rms = np.sqrt(np.einsum('ij,ij->i', chunks, chunks) / window) + 1e-6

            # Identify spikes in all chunks at once. Broadcast rms: (n_chunks, 1)
            spikes = np.abs(chunks) > (rms[:, None] * 10)

            out = wav.copy()
            if np.any(spikes):
                # Apply clamping only to spikes. We view 'out' in chunk form.
                out_full = out[:full_part_len].reshape(n_full_chunks, window)
                # Broadcast rms back to (n_chunks, window) for boolean indexing
            ...
            # Handle remainder (last partial window)
