...
## 2026-03-03 - [Vectorized De-Clicking Heuristic]
**Learning:** Replacing O(N) Python loops with vectorized NumPy operations (`reshape`, `einsum`) for audio de-clicking provides a ~9x speedup. A critical edge case was discovered: the "10x local RMS" spike detection heuristic only triggers for a single-sample spike if the window size $N > 100$ (since $\sqrt{N}$ must be $> 10$). At 24kHz (2ms = 48 samples), single spikes are mathematically impossible to detect unless accompanied by other artifacts or background noise is extremely low.
**Action:** Use vectorized NumPy patterns for window-based audio processing. Be aware of the mathematical limits of heuristics when window sizes are small relative to threshold factors.
