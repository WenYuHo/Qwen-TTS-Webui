...
class AudioPostProcessor:
    ...
    def apply_declick(wav: np.ndarray, sr: int) -> np.ndarray:
        """Simple heuristic de-clicker: clamps spikes > 10x local RMS using vectorized NumPy."""
        try:
            # ⚡ Bolt: Handle multi-channel audio by recursive application
            if len(wav.shape) > 1:
                return np.stack([AudioPostProcessor.apply_declick(ch, sr) for ch in wav])

            # ⚡ Bolt: Vectorized spike detection and clamping
            # 2ms window for local RMS calculation
            window = int(sr * 0.002)
            if window < 2 or len(wav) < window:
                return wav

            # 1. Prepare chunks (ignoring remainder for the main vectorized pass)
            n_chunks = len(wav) // window
            main_len = n_chunks * window
            chunks = wav[:main_len].reshape(n_chunks, window)

            # 2. Calculate local RMS for each chunk using np.einsum for efficiency
            # O(N) memory allocation avoided by not using chunks**2
            sq_sums = np.einsum('ij,ij->i', chunks, chunks)
            rms = np.sqrt(sq_sums / window) + 1e-6

            # 3. Detect spikes: amplitude > 10x local RMS
            # Broadcast RMS to chunk shape: (n_chunks, 1) -> (n_chunks, window)
            spikes = np.abs(chunks) > (rms[:, None] * 10)

            # 4. If any spikes detected, clamp them
            if np.any(spikes):
                out_chunks = chunks.copy()
                # Sign * local_rms * 3
                # We use np.where to get indices for the flattened mask or just assignment
                # Since chunks is a view of wav, we MUST copy if we want to return a copy.
                # The original implementation returns out = wav.copy()
                clamp_vals = rms * 3
                # ⚡ Bolt: Use row-indexing for memory-efficient clamping without large array allocation
                row_idx, _ = np.where(spikes)
                out_chunks[spikes] = np.sign(chunks[spikes]) * clamp_vals[row_idx]
                out = out_chunks.flatten()
            else:
                out = wav[:main_len].copy()

            # 5. Handle remainder (tail of the audio)
            if main_len < len(wav):
