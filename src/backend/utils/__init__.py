...
class AudioPostProcessor:
    ...
    def apply_declick(wav: np.ndarray, sr: int) -> np.ndarray:
        ...
        try:
            # ⚡ Bolt: Use np.stack for multi-channel processing to maintain consistency and performance.
            if len(wav.shape) > 1:
            ...
            out = wav.copy()
            n_chunks = len(out) // window
            main_len = n_chunks * window

            # ⚡ Bolt: Vectorize spike detection using np.reshape and np.einsum.
            # This avoids the O(N) Python loop and significantly improves throughput.
            chunks = out[:main_len].reshape(-1, window)

            # Row-wise RMS calculation: sqrt(sum(x**2)/N). einsum is more memory-efficient than chunks**2.
            ...
            if np.any(spikes):
                ...
                # row_idx maps the spiked sample back to its chunk's RMS value.
                row_idx, _ = np.where(spikes)
                clamp_vals = rms[row_idx] * 3
                chunks[spikes] = np.sign(chunks[spikes]) * clamp_vals
