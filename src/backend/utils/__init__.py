...
class AudioPostProcessor:
    ...
    def apply_declick(wav: np.ndarray, sr: int) -> np.ndarray:
        ...
        try:
            # ⚡ Bolt: Handle multi-channel by stacking vectorized mono calls
            if len(wav.shape) > 1:
            ...
            if n_windows > 0:
                ...
                chunks = out[:n_windows * window].reshape(n_windows, window)

                # ⚡ Bolt: Use einsum for memory-efficient row-wise squared sum (power)
                # This avoids allocating a temporary intermediate array for chunks**2.
                rms = np.sqrt(np.einsum('ij,ij->i', chunks, chunks) / window) + 1e-6
                rms_expanded = rms[:, np.newaxis]

                # Identify spikes across all windows at once
                spikes = np.abs(chunks) > (rms_expanded * 10)

                if np.any(spikes):
                    # ⚡ Bolt: Use broadcasting for efficient clamping. We broadcast the 1D RMS array
                    # to match the 2D chunk shape so boolean indexing can align correctly.
                    rms_broadcast = np.broadcast_to(rms_expanded, chunks.shape)
                    chunks[spikes] = np.sign(chunks[spikes]) * rms_broadcast[spikes] * 3

            # Handle remainder (tail of the audio)
            remainder = out[n_windows * window:]
            if len(remainder) >= 2:
                # ⚡ Bolt: Use vdot for remainder RMS to avoid temporary array allocation.
