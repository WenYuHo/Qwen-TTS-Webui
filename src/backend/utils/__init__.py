...
class AudioPostProcessor:
    ...
    def apply_declick(wav: np.ndarray, sr: int) -> np.ndarray:
        ...
        try:
            ...
            # ⚡ Bolt: Vectorized de-clicking.
            # 1. Reshape into non-overlapping windows (O(1) view)
            n_windows = len(wav) // window
            main_part = wav[:n_windows * window].reshape(n_windows, window)

            # 2. Calculate RMS per window (O(N) but vectorized)
            # Use np.einsum for squared sum to avoid O(N) temporary array allocation.
            rms = np.sqrt(np.einsum('ij,ij->i', main_part, main_part) / window) + 1e-6

            # 3. Detect spikes (broadcasting)
            # spikes is a boolean mask of shape (n_windows, window)
            ...
            has_rem_spikes = rem_spikes is not None and np.any(rem_spikes)

            if has_spikes or has_rem_spikes:
                # 5. Clamp spikes: sign * local_rms * 3
                # We only need to copy if we actually find spikes
                out = wav.copy()

                if has_spikes:
                    out_main = out[:n_windows * window].reshape(n_windows, window)
                    # Use indexing to apply clamping to spiked samples
                    row_idx, _ = np.where(spikes)
                    out_main[spikes] = np.sign(out_main[spikes]) * rms[row_idx] * 3

                if has_rem_spikes:
                    out[n_windows * window:][rem_spikes] = np.sign(remainder[rem_spikes]) * rem_rms * 3
                return out
