...
def test_find_model_path_exists(mock_models_path):
...
@patch("backend.config.MODELS_PATH")
def test_find_model_path_flattened(mock_models_path):
...
def test_env_variable_priority():
    ...
    with patch.dict(os.environ, {"COMFY_QWEN_MODELS_DIR": custom_path}):
        ...
        assert str(MODELS_PATH) == custom_path
...
def test_verify_system_paths(mock_models, mock_models_path):
    ...
    assert results["models_dir_exists"]
    assert "M1" in results["found_models"]
