...
## 2026-03-07 - [Optimized Attentive Statistics Pooling]
**Learning:** The `AttentiveStatisticsPooling` layer in the speaker encoder was performing redundant operations: generating a mask of ones for every pass, using sub-optimal variance formulas, and concatenating large tensors along the channel dimension before a 1x1 convolution. Since the projection of a concatenated tensor `Conv1d([x, mean, std])` is equivalent to `Conv1d(x) + Conv1d(mean) + Conv1d(std)`, and since `mean` and `std` are constant over time, their projections can be computed in O(1) time/space and broadcasted. This refactoring, combined with using native `torch.mean` and `torch.var`, yields a ~2.2x speedup on CPU.
**Action:** Always prefer broadcasting for time-invariant components in pooling layers. Utilize the $E[X^2] - (E[X])^2$ variance formula to minimize temporary tensor allocations in high-frequency ML kernels.
