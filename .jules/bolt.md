...
**Learning:** Replacing $O(N)$ Python loops for audio chunk processing (e.g., in a de-clicker) with vectorized NumPy operations using `reshape`, `einsum`, and advanced indexing provides a massive performance boost (~32x in this codebase). This reduces execution time for a 60s audio buffer from ~714ms to ~22ms. Using `np.einsum` for row-wise dot products is particularly efficient as it avoids large temporary array allocations.
**Action:** Always vectorize chunk-based audio processing tasks using NumPy's `reshape` and boolean indexing to avoid the overhead of Python loops.
