...
async function handleCloneUpload(files) {
    ...
    const nameEl = document.getElementById('clone-filename');
    if (nameEl) {
        nameEl.innerText = files.length > 1 ? `${files.length} samples selected` : files[0].name;
    }
}
...
function updateBlockUI(id) {
    ...
    if (el) {
        // Targeted DOM update for performance (O(1))
