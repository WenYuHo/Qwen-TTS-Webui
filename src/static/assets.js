...
export const AssetManager = {
    ...
    playAsset(name, btn) {
        ...
        // Restore previous button if it was still loading
        if (this._lastPlayBtn && this._lastPlayBtn !== btn && this._lastPlayBtn.dataset.original) {
            this._lastPlayBtn.innerHTML = this._lastPlayBtn.dataset.original;
        ...
        const cleanup = () => {
            if (btn) {
                ...
                btn.disabled = false;
                if (this._lastPlayBtn === btn) this._lastPlayBtn = null;
            }
        };

        player.src = `/api/assets/download/${name}`;
        player.onplay = cleanup;
        player.onerror = () => {
            cleanup();
