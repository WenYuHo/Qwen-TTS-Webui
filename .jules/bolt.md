...
## 2026-03-31 - [Vectorized De-clicker Optimization]
**Learning:** Python loops for per-chunk audio processing (e.g., 2ms windows) are a major bottleneck. For 60s of 24kHz audio, this involves ~30,000 iterations, taking ~680ms. Vectorizing this using NumPy's `reshape` and `einsum` for RMS calculation reduces execution time to ~87ms (~7.8x speedup) while maintaining bit-perfect parity.
**Action:** Always replace O(N) Python loops in audio DSP utilities with vectorized NumPy operations. Use `np.einsum('ij,ij->i', chunks, chunks)` for memory-efficient squared sums.
