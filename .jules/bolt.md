...
## 2026-03-18 - [Vectorized Audio De-clicking and Mask Indexing]
**Learning:** Replacing $O(N)$ Python loops with vectorized NumPy operations for chunk-based audio processing (like de-clicking) provides a ~30x-70x speedup. However, indexing a 1D auxiliary array (like RMS values) with a 2D boolean mask from a reshaped waveform requires using `np.where(mask)[0]` to retrieve the correct row-wise indices and avoid an `IndexError`. Additionally, the '10x local RMS' heuristic for spike detection is window-size dependent; for single spikes to trigger detection, the window size $N$ must satisfy $\sqrt{N} > \text{threshold factor}$.
**Action:** Use `row_idx, _ = np.where(mask)` when mapping multi-dimensional boolean masks back to row-wise calculation results in vectorized pipelines. Ensure test cases use sufficiently large windows or mock sample rates to functionally verify heuristic-based DSP logic.
