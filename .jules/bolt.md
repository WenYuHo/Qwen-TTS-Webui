...
## 2026-03-29 - [Vectorized Audio Heuristics]
**Learning:** Replacing O(N) Python loops in audio heuristics (like de-clicking) with vectorized NumPy operations (`reshape`, `np.einsum`) provides a massive performance boost (~45-50x speedup). Using `np.einsum('ij,ij->i', chunks, chunks)` for row-wise squared sums avoids allocating large temporary arrays, which is critical for memory efficiency on long audio buffers.
**Action:** Always vectorize O(N) audio processing loops using NumPy. Use `einsum` for memory-efficient row-wise operations.
