...
class AudioPostProcessor:
    ...
    def apply_declick(wav: np.ndarray, sr: int) -> np.ndarray:
        """⚡ Bolt: Vectorized de-clicker: clamps spikes > 10x local RMS.
        Replaces O(N) Python loop with efficient NumPy broadcasting and einsum.
        """
        try:
            if len(wav.shape) > 1:
            ...
            if n_chunks > 0:
                main_part = out[:n_chunks * window]
                chunks = main_part.reshape(n_chunks, window)

                # ⚡ Bolt: Use einsum for memory-efficient row-wise squared sum (RMS)
                ...
                if np.any(spikes):
                    # ⚡ Bolt: Only apply modifications where spikes exist to save cycles
                    row_idx, col_idx = np.where(spikes)
                    # Clamp spikes to local RMS * 3
                    out_chunks = out[:n_chunks * window].reshape(n_chunks, window)
                    out_chunks[row_idx, col_idx] = np.sign(chunks[row_idx, col_idx]) * rms[row_idx] * 3

            # Handle remainder
            remainder_start = n_chunks * window
            if remainder_start < len(wav) - 1:
