...
**Learning:** Vectorizing a windowed audio heuristic (like a spike de-clicker) using NumPy reshaping and `np.einsum` for RMS calculations eliminates the massive overhead of Python loops. For a 2ms window @ 24kHz, this reduces execution time by ~74x. Using `np.where(spikes)` to retrieve indices for vectorized clamping avoids unnecessary full-array operations.
**Action:** Always prefer reshaping and vectorized boolean indexing over Python loops for windowed audio DSP tasks.
