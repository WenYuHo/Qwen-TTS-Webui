...
**Learning:** Replacing O(N) Python loops with NumPy vectorization (using `reshape` and `einsum`) for audio heuristic processing like de-clicking can yield >10x speedups. Using row-indexing (`arr[mask] = values[row_indices]`) for element-wise assignment from auxiliary arrays is a memory-efficient way to handle conditional clamping without allocating large intermediate broadcasted arrays.
**Action:** Always look for opportunities to replace per-chunk or per-sample Python loops in audio processing with NumPy vectorized operations. Use `np.einsum` for efficient row-wise operations like squared sums.
