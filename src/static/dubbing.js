...
export const DubbingManager = {
    ...
    initDubPreview(filename) {
        ...
        playBtn.onclick = () => {
            ...
            playBtn.setAttribute('aria-label', isPlaying ? 'Pause source preview' : 'Play source preview');
            playBtn.setAttribute('title', isPlaying ? 'Pause source preview' : 'Play source preview');
        };

        this.wavesurfer.on('finish', () => {
            playBtn.innerHTML = '<i class="fas fa-play" aria-hidden="true"></i>';
            playBtn.setAttribute('aria-label', 'Play source preview');
            playBtn.setAttribute('title', 'Play source preview');
        });
    },
    ...
    setupS2SRecording() {
        ...
        btn.onclick = async () => {
            if (!window.state.s2s.isRecording) {
                ...
                btn.setAttribute('aria-label', 'Record source audio for voice changer');
                btn.setAttribute('title', 'Record source audio for voice changer');
