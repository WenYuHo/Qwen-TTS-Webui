...
class AudioPostProcessor:
    ...
    def apply_declick(wav: np.ndarray, sr: int) -> np.ndarray:
        ...
        # ⚡ Bolt: Vectorized implementation using np.reshape and np.einsum.
        # This replaces O(N) Python loops with C-optimized NumPy operations,
        # providing a ~10-15x speedup.
        try:
            if len(wav.shape) > 1:
                # ⚡ Bolt: Use list comprehension for cleaner multi-channel handling
            ...
            main_part_len = num_windows * window

            # 1. Reshape into non-overlapping windows
            chunks = out[:main_part_len].reshape(num_windows, window)
            
            # 2. ⚡ Bolt: Calculate row-wise RMS using np.einsum to avoid O(N) temporary squares.
            # RMS = sqrt(mean(x^2))
            # np.einsum('ij,ij->i', chunks, chunks) computes sum(x^2) for each row.
            rms = np.sqrt(np.einsum('ij,ij->i', chunks, chunks) / window) + 1e-6

            # 3. Identify spikes (row indices and column indices)
            # threshold is 10x local RMS
            thresholds = rms * 10
            spikes = np.abs(chunks) > thresholds[:, np.newaxis]

            if np.any(spikes):
                # 4. ⚡ Bolt: Clamp spikes to 3x local RMS
                # We need to broadcast the row-wise RMS to the chunk shape
                target_values = (rms * 3)[:, np.newaxis]
                # Apply signs to the target values
                clamped = np.sign(chunks) * target_values
                # Update only spike positions
                chunks[spikes] = clamped[spikes]

            # 5. Handle remainder (if any)
            if n > main_part_len:
                remainder = out[main_part_len:]
                if len(remainder) >= 2:
