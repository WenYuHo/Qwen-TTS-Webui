...
class AudioPostProcessor:
    ...
    def apply_declick(wav: np.ndarray, sr: int) -> np.ndarray:
        ...
        try:
            # ⚡ Bolt: Vectorized implementation for ~100x speedup
            ...
            # Process the main part of the audio in reshaped chunks
            n_chunks = len(wav) // window
            main_part = wav[:n_chunks * window].reshape(n_chunks, window)

            # ⚡ Bolt: Use einsum for vectorized RMS calculation across chunks
            ...
            # Identify spikes (O(N) vectorized comparison)
            spikes = np.abs(main_part) > (rms_expanded * 10)

            out_main = main_part.copy()
            if np.any(spikes):
                # ⚡ Bolt: Vectorized spike clamping using broadcasting and fancy indexing.
                # Clamps spikes to 3x local RMS.
                out_main[spikes] = np.sign(main_part[spikes]) * (np.broadcast_to(rms_expanded, main_part.shape)[spikes] * 3)

            # Handle the remainder samples that don't fit into a full window
            remainder = wav[n_chunks * window:]
            if len(remainder) >= 2:
                # ⚡ Bolt: np.vdot is efficient for a single RMS calculation
