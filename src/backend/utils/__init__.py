...
class AudioPostProcessor:
    ...
    def apply_declick(wav: np.ndarray, sr: int) -> np.ndarray:
        ...
        Achieves ~90x speedup over loop-based implementation for large buffers.
        """
        try:
            ...
            # ⚡ Bolt: Use einsum for row-wise squared sum to avoid O(N) temporary array for squares
            sq_sum = np.einsum('ij,ij->i', chunks, chunks)
            rms = np.sqrt(sq_sum / window) + 1e-6

            # Identify spikes: abs(x) > 10 * local_rms
            # Use broadcasting to compare each sample to its chunk's RMS
