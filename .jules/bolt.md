...
**Learning:** Replacing an O(N) Python loop with a vectorized NumPy implementation in the `apply_declick` heuristic provides an ~18.7x speedup (from 657ms to 35ms for 60s of audio). Using `np.reshape` and `np.einsum('ij,ij->i', chunks, chunks)` for row-wise dot products avoids redundant allocations of large temporary squared arrays, maintaining memory efficiency for long-form podcasts.
**Action:** Always prefer vectorized NumPy patterns with `einsum` or `vdot` over chunk-based Python loops for audio DSP tasks.
